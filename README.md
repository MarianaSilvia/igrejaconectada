...
- Agenda, comunicados, mural, atendimento pastoral, EBD, Discipulado, chamadas e canal privado com a liderança.
...
- Canal `Fale com a liderança` com encaminhamento por área, mensagens privadas, notificações e retenção de 90 dias após a conclusão.
...
Antes de liberar o canal privado para os membros, execute no Supabase SQL Editor:

```sql
-- arquivo do projeto
db/supabase-contact-leadership.sql
```

O canal usa tabelas próprias (`contact_destinations`, `contact_threads`, `contact_messages`, `contact_thread_reads` e `contact_audit`) e não grava conversas dentro do `church_app_state`.
Somente remetente, responsável e administrador geral podem ler uma conversa encaminhada. Conversas concluídas há mais de 90 dias são removidas pela rota `/api/contact/maintenance`, protegida por `CRON_SECRET`.

## Rotinas automaticas

Enquanto o backend ainda estiver na Vercel, o projeto usa Vercel Cron para:

- `/api/contact/maintenance`: lembrete diário de respostas pendentes e limpeza de conversas concluídas há mais de 90 dias.
