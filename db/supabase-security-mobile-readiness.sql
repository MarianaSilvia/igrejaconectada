...
drop policy if exists "users manage own chat blocks" on public.chat_blocks;
create policy "users manage own chat blocks" on public.chat_blocks
for all to authenticated
using (blocker_id = (select auth.uid()))
with check (blocker_id = (select auth.uid()));

create index if not exists chat_messages_user_id_idx on public.chat_messages (user_id);
create index if not exists chat_blocks_blocked_user_id_idx on public.chat_blocks (blocked_user_id);
create index if not exists chat_messages_hidden_by_idx on public.chat_messages (hidden_by);
create index if not exists chat_moderation_message_id_idx on public.chat_moderation (message_id);
create index if not exists chat_moderation_room_id_idx on public.chat_moderation (room_id);
create index if not exists chat_moderation_actor_id_idx on public.chat_moderation (actor_id);

drop policy if exists "chat rooms visible by congregation" on public.chat_rooms;
create policy "chat rooms visible by congregation" on public.chat_rooms
...
using (
  ...
  and (
    ...
    or exists (
      ...
      where m.auth_user_id = (select auth.uid()) and m.congregation = chat_rooms.congregation
    )
  )
);

drop policy if exists "chat messages visible by congregation" on public.chat_messages;
create policy "chat messages visible by congregation" on public.chat_messages
for select to authenticated
using (
  status = 'visible'
  and coalesce(((select auth.jwt()) -> 'app_metadata' ->> 'church_gp_access'), '') = 'approved'
  and (
    coalesce(((select auth.jwt()) -> 'app_metadata' ->> 'church_gp_role'), '') = 'ADMIN'
    or congregation = coalesce(((select auth.jwt()) -> 'app_metadata' ->> 'church_gp_congregation_scope'), '')
    or exists (
      select 1 from public.members m
      where m.auth_user_id = (select auth.uid()) and m.congregation = chat_messages.congregation
    )
  )
);

drop policy if exists "chat moderation visible to admins" on public.chat_moderation;
create policy "chat moderation visible to admins" on public.chat_moderation
for select to authenticated
using (
  coalesce(((select auth.jwt()) -> 'app_metadata' ->> 'church_gp_access'), '') = 'approved'
